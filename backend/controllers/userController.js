    ...
    if(data.data.list){
      ...
      for (let index = 0; index < txList.length; index++) {
        ...
        if(bitcoinTxHashes.indexOf(txHash) == -1){
          amount = tx.balance_diff * Math.pow(10, -8);
...
const withdraw = asyncHandler(async (req, res) => {
  ...
  if(type == "eth" || type == "btc") {
    try{
      ...
      const networkConfig = type == 'eth' ? 'eth' : 'btc';
      // const account = new CryptoAccount(houseWalletPrivateKey, {network: networkConfig});
      const account = new CryptoAccount(houseWalletPrivateKey);
  ...
  if(type == "sol") {
    ...
    const networkConfig = "mainnet-beta"; //mainnet-beta
